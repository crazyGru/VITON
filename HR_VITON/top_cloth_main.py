...
def try_on(person_folder, cloth_folder, data_folder):
    ...
    cmd = f"python test_generator.py --occlusion --cuda False --tocg_checkpoint checkpoints/mtviton.pth \
...
# openpose(r'tmp\test', output_folder)
...
# input_person2 = "test_image/2.jpg"
...
def get_cloth(input_path, output_path):
    ...
    for image_path in images_list:
        ...
        mask[output_arr == 1] = np_img[output_arr == 1]
        image_name = os.path.basename(image_path)
        cv2.imwrite(os.path.join(output_dir, image_name[:-4] + "_cloth.jpg"), mask)
        pbar.update(1)
    pbar.close()

get_cloth(input_person1, "tmp/test")
# get_cloth(input_person2, "tmp/test")